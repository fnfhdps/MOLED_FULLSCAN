#pragma once

#include <iostream>
#include <cmath>
#include <string>
#include <random>
#include <memory>
#include <algorithm>
#include <vector>
#include <omp.h>

#include "AbstractModel.h"
#include "..\RunningStats.h"

namespace GRANSAC
{
	// T - AbstractModel
	template <class T, int t_NumParams>
	class RANSAC
	{
	private:
		std::vector<std::shared_ptr<dPoint>> m_Data;				// All the data

		std::vector<std::shared_ptr<T>>				m_SampledModels;	// Vector of all sampled models
		std::shared_ptr<T>							m_BestModel;		// Pointer to the best model, valid only after Estimate() is called
		std::vector<std::shared_ptr<dPoint>>		m_BestInliers;		// 최적 라인을 구성하는 점들의 집합. Threshold 조건을 만족하는 점들을 의미

		int		m_MaxIterations;	// Number of iterations before termination
		double	m_Threshold;		// The threshold for computing model consensus
		double	m_BestModelScore;	// The score of the best model(= Threshold 만족 데이터 / 전체 데이터)
		int		m_BestModelIdx;

		std::vector<std::mt19937> m_RandEngines; // Mersenne twister high quality RNG that support *OpenMP* multi-threading

	public:
		RANSAC(void)
		{
			int nThreads = max(1, omp_get_max_threads());
			std::cout << "[ INFO ]: Maximum usable threads: " << nThreads << std::endl;

			for (int i = 0; i < nThreads; ++i)
			{
				std::random_device SeedDevice;
				m_RandEngines.push_back(std::mt19937(SeedDevice()));
			}

			Reset();
		};

		virtual ~RANSAC(void) {};

		void Reset(void)
		{
			// Clear sampled models, etc. and prepare for next call. Reset RANSAC estimator state
			m_Data.clear();
			m_SampledModels.clear();

			m_BestModelIdx = -1;
			m_BestModelScore = 0.0;
		};

		/// <summary>
		/// RANSAC 모델 초기화.
		/// </summary>
		/// <param name="Threshold">제공된 데이터들 중에 모델과 데이터간 허용 거리. Threshold를 초과하는 데이터 모델 불만족.</param>
		/// <param name="MaxIterations">최대 반복 회수.</param>
		// N = log(1 - p) / log(1 - a^m) = log(1-0.999) / log(1-0.8^5) ;
		// N : 반복 회수
		// p : 기대 확률.(기대 정확도)
		// a : inlier의 비율.(전제 데이터중 신뢰 데이터 비율.
		// m : sampling 수량.
		//int max_iteration = (int)(log(1. - 0.99) / log(1. - pow(0.5, no_samples)) + 0.5);

		//threshold 값 결정
		//threshold는 inlier데이터의 분산과 관련이 있음.
		//정확히 fitting된 결과와 나머지 inlier의 거리에 대한 분산을 구하면
		//threshold값을 어느 정도 범위에서 결정할 것인지 정할 수 있음.
		// 2sigma : 97.7%, 3sigma : 99.9%의 inlier들을 포함하게 됨.
		void Initialize(double Threshold, int MaxIterations = 1000)
		{
			m_Threshold = Threshold;
			m_MaxIterations = MaxIterations;
		};

		std::shared_ptr<T> GetBestModel(void) { return m_BestModel; }

		const std::vector<std::shared_ptr<dPoint>>& GetBestInliers(void) { return m_BestInliers; }

		double GetBestModelScore(void) { return m_BestModelScore; }

		bool Estimate(std::vector<std::shared_ptr<dPoint>> Data, double r = 0.)
		{
			Reset();

			if (Data.size() <= t_NumParams)
			{
				std::cout << "[ WARN ]: RANSAC - Number of data points is too less. Not doing anything." << std::endl;
				return false;
			}

			m_Data = Data;
			size_t DataSize = m_Data.size();
			std::uniform_int_distribution<int> UniDist(0, int(DataSize - 1)); // Both inclusive

			std::vector<double> InlierFractionAccum(m_MaxIterations);
			std::vector<std::vector<std::shared_ptr<dPoint>>> InliersAccum(m_MaxIterations);
			m_SampledModels.resize(m_MaxIterations);

			int nThreads = max(1, omp_get_max_threads());
			omp_set_dynamic(0); // Explicitly disable dynamic teams
			omp_set_num_threads(nThreads);

#pragma omp parallel for
			for (int i = 0; i < m_MaxIterations; ++i)
			{
				// Select t_NumParams random samples
				std::vector<std::shared_ptr<dPoint>> RandomSamples(t_NumParams);
				std::vector<std::shared_ptr<dPoint>> RemainderSamples = m_Data; // Without the chosen random samples

				std::shuffle(RemainderSamples.begin(), RemainderSamples.end(), m_RandEngines[omp_get_thread_num()]); // To avoid picking the same element more than once
				std::copy(RemainderSamples.begin(), RemainderSamples.begin() + t_NumParams, RandomSamples.begin());
				RemainderSamples.erase(RemainderSamples.begin(), RemainderSamples.begin() + t_NumParams);

				std::shared_ptr<T> RandomModel = std::make_shared<T>(RandomSamples, r);

				// Check if the sampled model is the best so far
				std::pair<double, std::vector<std::shared_ptr<dPoint>>> EvalPair = RandomModel->Evaluate(RemainderSamples, m_Threshold);
				InlierFractionAccum[i] = EvalPair.first;
				InliersAccum[i] = EvalPair.second;

				// Push back into history. Could be removed later
				m_SampledModels[i] = RandomModel;
			}

			for (int i = 0; i < m_MaxIterations; ++i)
			{
				if (InlierFractionAccum[i] > m_BestModelScore)
				{
					m_BestModelScore= InlierFractionAccum[i];
					m_BestModelIdx	= (int)(m_SampledModels.size() - 1);
					m_BestModel		= m_SampledModels[i];
					m_BestInliers	= InliersAccum[i];
				}
			}
			
			if (m_BestModel != nullptr)
			{
				CRunningStats overall, inlier;

				for (auto& param : m_BestInliers)
				{
					double dist = m_BestModel->ComputeDistanceMeasure(param);
					inlier.Push(dist);
				}

				m_BestModel->m_inlierTh = inlier.StandardDeviation();

				for (auto& param : Data)
				{
					double dist = m_BestModel->ComputeDistanceMeasure(param);
					overall.Push(dist);
				}

				m_BestModel->m_totalTh = overall.StandardDeviation();
			}
			
			// std::cout << "BestInlierFraction: " << m_BestModelScore << std::endl;

			return true;
		};
	};
} // namespace GRANSAC
